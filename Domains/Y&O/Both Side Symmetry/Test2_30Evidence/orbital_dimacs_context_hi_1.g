...
context_vars_list :=[];;
context_vars_list[1] := sizeOfElements;;
...
out_file :=OutputTextFile(Concatenation("CON-MCMC_HeuristicI_1_",String(pBias),String(sizeOfElements),".csv"),false);;
...
for iter in [1..sizeArr] do
	...
	klArr[iter] := 0;;
od;;
weightPerElement := (1-pBias)/sizeOfElements;;

weightProb :=[];;
for i in [1..sizeOfElements] do
	weightProb[i] := i;;
od;;
for i in [1..Int(pBias/weightPerElement)+1] do
	weightProb[sizeOfElements+i] := context_vars_list[1];;
od;;
...
# the evidence is always part of the samples
	...
	while sampleCounter < numSamples do
		...
		bNr:= weightProb[Int(pVal/weightPerElement+1)];;
		...
		# print something every 10000 samples
			if RemInt(sampleCounter, interval) = 0 then
				...
				#Print("herhe",sampleCounter/interval,"\n");;
				# if sampleCounter/interval =0 then
				# 	continue;;
				# fi;;
