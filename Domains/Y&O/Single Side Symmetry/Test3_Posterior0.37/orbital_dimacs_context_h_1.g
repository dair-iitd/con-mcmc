...
out_file :=OutputTextFile(Concatenation("CON-MCMC_Heuristic_1_",String(pBias),String(sizeOfElements),".csv"),false);;
...
for iter in [1..sizeArr] do
	...
	klArr[iter] := 0;;
od;;
contextProb := pBias+(1-pBias)/sizeOfElements;;
...
# the evidence is always part of the samples
	...
	while sampleCounter < numSamples do
		...
		if (pVal <= 1-contextProb) then
			bNr := Int(pVal*(sizeOfElements-1)/(1-contextProb))+1;;
		else
			bNr := context_vars_list[1];;
		fi;;
