...
out_file := OutputTextFile("marginals.g",false);;
AppendTo(out_file,"correctMargs:=[\n");;

for i in [1..sizeOfElements] do
	 if i<>sizeOfElements then
	 	AppendTo(out_file,String(0),",\n");;
	 else
	 	AppendTo(out_file,String(0),"\n");;
	 fi;;
od;;
...
out_file :=OutputTextFile(Concatenation("KL_results_PureGibbs_",String(sizeOfElements),".csv"),false);;
...
for iter in [0..maxIterations-1] do
	...
	while sampleCounter < numSamples do
		...
		bNr := Random(rs1, 1, numOfBlocks-1);;
		...
		if RemInt(sampleCounter, interval) = 0 then
			...
			out_file1 := OutputTextFile("marginalsfs2.g",false);;
		 	AppendTo(out_file1,"correctMargs:=[\n");;

	 		for i in [1..sizeOfElements] do
	 			if i<>sizeOfElements then
	 				AppendTo(out_file1,String(tmpMargs[i]),",\n");;
	 			else
	 				AppendTo(out_file1,String(tmpMargs[i]),"\n");;
	 			fi;;
	 		od;;
			AppendTo(out_file1,"];;");;
			CloseStream(out_file1);;
