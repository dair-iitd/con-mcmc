...
out_file :=OutputTextFile(Concatenation("Vanilla2-MCMC_",String(sizeOfElements),".csv"),false);;
...
CloseStream(out_file);;
	...
	#  out_file := OutputTextFile("marginalsfs.g",false);;
	#  AppendTo(out_file,"correctMargs:=[\n");;

	#  for i in [1..sizeOfElements] do
	#  	if i<>sizeOfElements then
	#  		AppendTo(out_file,String(tmpMargs[i]),",\n");;
	#  	else
	#  		AppendTo(out_file,String(tmpMargs[i]),"\n");;
	#  	fi;;
	#  od;;
